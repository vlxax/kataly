...
for(let t=0;t<500;t++){
  ...
  const g=new HoldemDemo({heroNick:'P0',players:players.map(p=>({nick:p.nick})),stackBB:50,levelSeconds:99999,botDelayMs:0,eventPaceMs:0});
  g._testSetState({players,pot:players.reduce((s,p)=>s+p.totalBet,0)});
  ...
  for(const pot of pots)assert.ok(pot.eligible.length>0);
  g.destroy();
}
