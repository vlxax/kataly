...
function fiveRank(cards){const v=cards.map(c=>RANK_VALUE[c[0]]).sort((a,b)=>b-a),s=cards.map(c=>c[1]),ct={};v.forEach(x=>ct[x]=(ct[x]||0)+1);const g=Object.entries(ct).map(([x,n])=>({v:+x,n})).sort((a,b)=>b.n-a.n||b.v-a.v),u=[...new Set(v)].sort((a,b)=>b-a);if(u[0]===14)u.push(1);let sh=0;for(let i=0;i<=u.length-5;i++)if(u[i]-u[i+4]===4){sh=u[i];break}const f=new Set(s).size===1;if(f&&sh)return[8,sh];if(g[0].n===4)return[7,g[0].v,g[1].v];if(g[0].n===3&&g[1] && g[1].n>=2)return[6,g[0].v,g[1].v];if(f)return[5,...v];if(sh)return[4,sh];if(g[0].n===3)return[3,g[0].v,...g.filter(x=>x.n===1).map(x=>x.v).sort((a,b)=>b-a)];if(g[0].n===2&&g[1] && g[1].n===2){const p=[g[0].v,g[1].v].sort((a,b)=>b-a);return[2,...p,((g.find(x=>x.n===1)||{}).v||0)]}if(g[0].n===2)return[1,g[0].v,...g.filter(x=>x.n===1).map(x=>x.v).sort((a,b)=>b-a)];return[0,...v]}
...
export const rankLabel=r=>['Старшая карта','Пара','Две пары','Сет','Стрит','Флеш','Фулл-хаус','Каре','Стрит-флеш'][(r && r[0])||0];
...
export class HoldemDemo{
 ...
 updateLevel(){const elapsed=Math.floor((Date.now()-this.levelStartedAt)/1000),target=Math.min(this.blindSchedule.length-1,Math.floor(elapsed/this.levelSeconds));if(target!==this.level){this.level=target;const x=this.blindSchedule[target];this.sb=x.sb;this.bb=x.bb;this.ante=x.ante!=null?x.ante:x.bb;this.lastFullRaise=this.bb}}
 ...
 snapshot(){const avg=this.active().reduce((s,p)=>s+p.stack,0)/Math.max(1,this.active().length);return{players:this.players.map(p=>({...p,hole:p.nick===this.heroNick?p.hole:['XX','XX'],stackBB:p.stack/this.bb,betBB:p.bet/this.bb})),heroHole:(this.hero() && this.hero().hole)||[],board:[...this.board],pot:this.pot,potBB:this.pot/this.bb,street:this.street,handNo:this.handNo,currentBet:this.currentBet,button:this.button,log:[...this.log].slice(-8),handActions:[...this.handActions],sb:this.sb,bb:this.bb,ante:this.ante,level:this.level+1,levelRemaining:this.levelRemaining(),nextLevel:this.nextLevel(),activePlayers:this.active().length,totalPlayers:this.players.length,averageStackBB:avg/this.bb,heroStackBB:((this.hero() && this.hero().stack)||0)/this.bb,eliminations:[...this.eliminations],finished:this.finished}}
 ...
 async botAction(p,l){
  ...
  const st=this.preflopStrength(p.hole);
  const pressure=l.toCall/Math.max(1,p.stack+l.toCall);

  if(l.toCall>0 && st<0.30 && pressure>0.04){
   return {type:'fold'};
  }

  if(l.canRaise && st>0.73 && Math.random()<0.48){
   const target=Math.min(
    l.maxRaise,
    Math.max(
     l.minRaise,
     this.currentBet+Math.round(Math.max(this.bb*1.2,this.pot*0.55))
    )
   );
   return {type:'raise',amount:target};
  }

  if(l.toCall===0 && l.canRaise && st>0.57 && Math.random()<0.30){
   return {
    type:'raise',
    amount:Math.min(
     l.maxRaise,
     Math.max(l.minRaise,Math.round(this.pot*0.5))
    )
   };
  }

  return {type:l.canCheck?'check':'call'};
 }
 preflopStrength(h){const a=RANK_VALUE[h[0][0]],b=RANK_VALUE[h[1][0]],pair=a===b,suited=h[0][1]===h[1][1];return Math.min(1,(a+b)/30+(pair ? 0.25 : 0)+(suited ? 0.07 : 0)-(Math.abs(a-b) > 5 ? 0.08 : 0))}
 ...
 finishHand(){const awards=this.awardPots(),winners=[...new Set([].concat.apply([],awards.map(x=>x.winners)))];const newlyOut=[];this.players.forEach(p=>{if(p.stack<=0&&!p.out){p.out=true;const place=this.active().length+1,e={nick:p.nick,place,handNo:this.handNo,ts:Date.now()};this.eliminations.push(e);newlyOut.push(e)}});const summary={handNo:this.handNo,pot:this.pot,potBB:this.pot/this.bb,winners,sidePots:awards,board:[...this.board],heroHole:[...((this.hero() && this.hero().hole)||[])],actions:[...this.handActions],label:winners.length?rankLabel((this.players.find(p=>p.nick===winners[0])||{}).showRank):'',sb:this.sb,bb:this.bb,ante:this.ante,level:this.level+1,newlyOut};this.sessionHands.push(summary);this.log.push(`Банк ${this.pot.toLocaleString('ru-RU')} → ${winners.join(', ')}`);this.emit();if(this.onHandEnd)this.onHandEnd(summary);if(this.active().length<=1||this.hero() && this.hero().out)this.finishTournament()}
 finishTournament(){if(this.finished)return;this.finished=true;clearInterval(this.timer);const active=this.active();if(active.length===1&&!this.eliminations.some(e=>e.nick===active[0].nick))this.eliminations.push({nick:active[0].nick,place:1,handNo:this.handNo,ts:Date.now()});const h=this.eliminations.find(e=>e.nick===this.heroNick);if(this.onTournamentEnd)this.onTournamentEnd({heroPlace:h && h.place||(active[0] && active[0].nick===this.heroNick?1:this.active().length+1),totalPlayers:this.players.length,winner:active.length===1?active[0].nick:null,eliminations:[...this.eliminations].sort((a,b)=>a.place-b.place),handNo:this.handNo,level:this.level+1,sb:this.sb,bb:this.bb,ante:this.ante})}
}
