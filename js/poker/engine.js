...
export class HoldemDemo {
  constructor({players, heroNick, stackBB=100, smallBlind=1, bigBlind=2, onChange, onHeroDecision, onHandEnd}){
    ...
    this.onChange=onChange;this.onHeroDecision=onHeroDecision;this.onHandEnd=onHandEnd;
    this.handNo=0;this.log=[];this.handActions=[];this.running=false;
  }
  ...
  async startHand(){
    ...
    if(this.active().length<2){this.running=false;return}
    this.handNo++; this.deck=shuffle(createDeck());this.board=[];this.pot=0;this.street='preflop';this.handActions=[];this.log=[];
  ...
  applyAction(p,a,toCall){
    ...
    this.handActions.push({handNo:this.handNo,street:this.street,player:p.nick,seat:p.seat,action:a.type,amountBB:amount,toCallBB:toCall,potAfterBB:this.pot,stackAfterBB:p.stack,ts:Date.now()});
  ...
  finishHand(){
    ...
    if(live.length===1) winners=live;
      ...
      let best=null;
      for(const p of live){p.showRank=evaluate7([...p.hole,...this.board]);if(!best||cmp(p.showRank,best)>0){best=p.showRank;winners=[p]}else if(cmp(p.showRank,best)===0)winners.push(p)}
    }
    const share=this.pot/winners.length; winners.forEach(w=>w.stack+=share);
    const summary={handNo:this.handNo,pot:this.pot,winners:winners.map(w=>w.nick),board:[...this.board],heroHole:[...(this.hero()?.hole||[])],actions:[...this.handActions],label:winners.length?rankLabel(winners[0].showRank):''};
    this.players.forEach(p=>{if(p.stack<=0)p.out=true});
    this.log.push(`Банк ${this.pot.toFixed(1)} BB → ${winners.map(w=>w.nick).join(', ')}`);
