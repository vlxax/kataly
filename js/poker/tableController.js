...
export class TableController{
  constructor({root,lobby,heroNick,onExit,onSessionEnd}){
    ...
    this.cancelled=false;this.lastSnapshot=null;this.lastHand=null;this.pendingResolve=null;this.pendingLegal=null;
  ...
  bindEvents(){
    ...
    on('TURN_STARTED',e=>{this.view.setTurn(e);if(e.nick!==this.heroNick)this.view.showWaiting(`${e.nick} думает…`)});
    ...
    on('BETTING_ROUND_COMPLETE',()=>this.view.showWaiting('Собираем ставки…'));
  ...
  onHeroDecision(legal,resolve){
    ...
    this.view.renderHeroControls(legal,action=>this.submitHero(action));
  }
  ...
  exit(){
    ...
    this.engine.destroy();this.root.remove();if(this.onExit)this.onExit();
  }
}
